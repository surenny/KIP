...
\begin{theorem}[Generalized Leibniz Rule]\label{thm:gen-leibniz}
    \uses{prop:delta-ess-adams,prop:cross-dr-En,prop:cross-f-Er,prereq:cor:four-spectra,def:f-Er-extension}
...
\begin{proof}
    ...
    Therefore, we can apply Corollary \ref{prereq:cor:four-spectra} and obtain
...
\begin{remark}
    We can further generalize Theorem \ref{thm:gen-leibniz} by using the conditions in Theorem \ref{prereq:thm:four-spectra} rather than those Corollary \ref{prereq:cor:four-spectra}. We leave this generalization to the reader.
\end{remark}

\begin{theorem}[Generalized Leibniz Rule, level $k$]\label{thm:gen-leibniz-k}
    \uses{prop:delta-ess-adams,prop:cross-dr-En,prop:cross-f-Er-level,prereq:cor:four-spectra,def:f-Er-level-extension}
    Let $f: X\to Y$ be a map between two classical spectra.
    Suppose that $2\le n\le r$, $e(f)\le m\le n-2+e(f)$, $l\ge e(f)$, $k\ge 0$, and we have
    $$x\in Z_{r-1}^{s,t}(X), \hspace{0.5cm} y\in Z_{r-1-m+e(f)}^{s+m,t+m}(Y)$$
    $$x_\infty\in Z_\infty^{s+r,t+r-1}(X), \hspace{0.5cm} y_\infty\in Z_\infty^{s+r+l,t+r+l-1}(Y)$$
    and the following conditions hold:
    \begin{enumerate}
        ...
        \item the differential in (1) has no crossing on the $E_n$-page or (2) has no crossing.
        \item the differential in (3) has no crossing.
    \end{enumerate}
    Then we have an Adams differential
    $$d_{r+l-m}(y)= y_\infty.$$
\end{theorem}
...
\begin{proof}
    ...
    \end{CD}$$
    By condition (1) and Proposition \ref{prop:delta-ess-adams}, we have
    \begin{equation}\label{eq:leibniz-k-delta-diff}
        d^{\delta_X}_r(x)=\lambda^{r-n}x_\infty.
    \end{equation}
    By conditions (2) and (3), and Definition \ref{def:f-Er-level-extension}, we have
    \begin{equation}\label{eq:leibniz-k-fhat-diff}
    ...
    \end{equation*}
    which implies
    \begin{equation}\label{eq:leibniz-k-fhat-xinfty}
         d^{\hat f}_l(\lambda^{r-n+k}x_\infty)=\lambda^{r+l-n-e(f)+k}y_\infty.
    \end{equation}

    Applying Proposition \ref{prop:cross-dr-En} and Proposition \ref{prop:cross-f-Er-level} to conditions (4) and (5), we know that the differential in (\ref{eq:leibniz-k-delta-diff}) or (\ref{eq:leibniz-k-fhat-diff}) has no crossing, and that the differential in (\ref{eq:leibniz-k-fhat-xinfty}) has no crossing.
...
\begin{theorem}[Generalized Leibniz Rule, first direction]\label{thm:gen-leibniz-k-fwd}
    \uses{prop:delta-ess-adams,prop:cross-dr-En,prop:cross-f-Er-level,prereq:cor:four-spectra,def:f-Er-level-extension}
    Let $f: X\to Y$ be a map between two classical spectra.
    Suppose that $2\le n\le r$, $e(f)\le m\le n-2+e(f)$, $l\ge e(f)$, $k\ge 0$, and we have
    $$x\in Z_{r-1}^{s,t}(X), \hspace{0.5cm} y\in Z_{r-1-m+e(f)}^{s+m,t+m}(Y)$$
...
\begin{proof}
    ...
    \end{CD}$$
    By condition (1) and Proposition \ref{prop:delta-ess-adams}, we have
    \begin{equation}\label{eq:leibniz-fwd-delta-diff}
        d^{\delta_X}_r(\lambda^k x)=\lambda^{r-n+k}x_\infty.
    \end{equation}
    By condition (2) and Definition \ref{def:f-Er-level-extension}, we have
    \begin{equation}\label{eq:leibniz-fwd-fhat-diff}
        d^{\hat f_{n-1}}_m(\lambda^k x)=\lambda^{m-e(f)+k} y.
    \end{equation}
    By condition (3) and Remark \ref{rmk:delta-ess-equiv-classical}, we have
    \begin{equation}\label{eq:leibniz-fwd-delta-y}
        d^{\delta_Y}_{r+l-m}(\lambda^{m-e(f)+k} y)=\lambda^{r+l-n-e(f)+k} y_\infty.
    \end{equation}

    Applying Proposition \ref{prop:cross-dr-En} and Proposition \ref{prop:cross-f-Er-level} to conditions (4) and (5), we know that the differential in (\ref{eq:leibniz-fwd-delta-diff}) or (\ref{eq:leibniz-fwd-fhat-diff}) has no crossing, and that the differential in (\ref{eq:leibniz-fwd-delta-y}) has no crossing.

    Therefore, we can apply Corollary \ref{prereq:cor:four-spectra} to the transposed commutative diagram (with $\delta_X$ as the first map, $\hat f_{n-1}$ as the second map, and $\delta_Y$ as the known target extension) and obtain
    $$d^{\hat f}_l(\lambda^{r-n+k}x_\infty)=\lambda^{r+l-n-e(f)+k}y_\infty.$$
...
\begin{theorem}[Generalized Mahowald Trick]\label{thm:gen-mahowald}
    ...
    \begin{enumerate}
        ...
        \item for $0 \le i \le n-1$, the Adams $E_\infty$-page of $Y$ vanishes at the positions that are potential targets of $(f,E_r)$-extensions of degree $i$ from $x$: $E_\infty^{s+l+i, t+l+i-1}(Y) = 0$.
    \end{enumerate}
...
\begin{proposition}\label{prop:ext-across-pages}
    \uses{prereq:cor:ess-naturality,def:f-Er-extension}
...
\begin{example}\label{exam:stretchext}
    ...
    for the map $f=\nu:S^3 \rightarrow S^0$, as discussed in Example~\ref{exam:extonEn}(2). However, to apply the Generalized Leibniz Rule in proving the classical Adams differential
    $$d_3(h_2h_5) = h_0p$$
    in Example~\ref{exam:Leibnizyes}, we need the $E_\infty$-page version of the $(f, E_3)$-extension:
    $$d_2^{f, E_\infty} (h_0h_4^2) = h_0 p.$$
    We apply Corollary~\ref{cor:stretch-extension} to confirm this.

    As checked in Example~\ref{exam:Ercross}(2), the $(f, E_3)$-extension has no crossing. Consequently, by Corollary~\ref{cor:stretch-extension}, we obtain the required $(f, E_\infty)$-extension:
    $$d_2^{f, E_\infty} (h_0h_4^2) = h_0 p.$$
\end{example}
