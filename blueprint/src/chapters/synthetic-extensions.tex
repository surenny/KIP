...
%where $\ZESS{f}_*^{*,*,*}$ and $\BESS{f}_*^{*,*,*}$ are  analogous to the notations in Notation \ref{prereq:not:representatives}, with the addition of an extra degree.

%In fact all the definitions and results in Section \ref{sec:prereq-ess} remain valid in this synthetic setting, provided we include the extra weight degree in addition to the topological degree and ensure that all maps also preserve the weight degree. For convenience, we will not restate these results here but will occasionally refer to them and apply their synthetic versions when needed.
...
\begin{proposition}\label{prop:lambda-rho-trivial}
    \uses{prereq:prop:syn-einfty-nuX,prereq:prop:syn-einfty-mod-lambda,prereq:cor:composition-zero,not:deltaandrho}
...
\begin{proof}
    ...
    Thus, we have shown that (\ref{eq:lambda-rho-exact}) is always exact in the middle. Combined with Corollary \ref{prereq:cor:composition-zero} we conclude that all $d^{\lambda^n}_i$, $d^{\rho}_i$ are trivial for $i>0$.
\end{proof}
...
\begin{proposition}\label{prop:delta-ess-adams}
    \uses{prereq:ax:lambda-bockstein,prereq:cor:triangle-map,prereq:cor:composition-ext,not:deltaandrho,prereq:prop:syn-einfty-nuX,prereq:prop:syn-einfty-mod-lambda}
...
\subsection{Delta ESS and Adams differentials}
...
\begin{remark}\label{rmk:delta-ess-equiv-classical}
    ...
    $$B_{r-1}^{s+r,t+r-1}(X)$$
    which is the same as the value of the classical Adams differential $d_r(x)=y$. This implies that the equation (\ref{eq:delta-ess-formula}) holds and is essential if and only if $d_r(x)=y$ holds and is essential. Therefore the $\delta$-ESS encodes the same information as the classical Adams spectral sequence.
\end{remark}

\subsection{Crossings on the $E_r$-page}
...
\subsection{Comparison of crossings}
...
\begin{remark}
    ...
    $d_r^{\delta_n}(\lambda^a x) = \lambda^{a+r-n-1}y$ also has no crossing.
\end{remark}
