...
\begin{definition}\label{prereq:def:einfty-data}
    ...
    \lean{KIP.SpectralSequence.EInftyData}\leanok
    The \emph{$E_\infty$-data} of a spectral sequence $E$ consists of a graded object $E_\infty^{\mathbf{k}}$ together with natural maps from each page $E_r^{\mathbf{k}} \to E_\infty^{\mathbf{k}}$ (for $r$ sufficiently large in the degenerate case), compatible with the page isomorphisms. For bounded or degenerate spectral sequences, $E_\infty = E_r$ for all sufficiently large $r$.
\end{definition}

\begin{definition}\label{prereq:def:ss-morphism}
    \uses{prereq:def:spectral-sequence}
    \lean{KIP.SpectralSequence.SpectralSequenceMorphism}\leanok
    A \emph{morphism of spectral sequences} $f: E \to E'$ (with the same differential degree function) consists of a family of graded maps $f_r: E_r^{\mathbf{k}} \to E'^{\mathbf{k}}_r$ commuting with differentials:
...
\begin{definition}\label{prereq:def:convergence}
    \uses{prereq:def:einfty-data,prereq:def:filtration,prereq:def:associated-graded}
...
\begin{definition}\label{prereq:def:converging-ss-morphism}
    \uses{prereq:def:convergence,prereq:def:filtration}
    \lean{KIP.SpectralSequence.ConvergenceMorphism}\leanok
    Given a reindexing rule, the \emph{category of converging spectral sequences} has objects $(E, A, F, \varphi)$ where $E$ is a spectral sequence, $A$ is a filtered graded object, and $\varphi: E_\infty \xrightarrow{\sim} \mathrm{gr}\, A$ is the convergence isomorphism. A morphism $(E_1, A_1) \to (E_2, A_2)$ consists of a morphism of spectral sequences $E_1 \to E_2$ together with a filtration-preserving map $A_1 \to A_2$, compatible with the convergence isomorphisms.
\end{definition}
...
\begin{definition}\label{prereq:def:filtration}
    ...
    A morphism of filtered graded abelian groups is a graded homomorphism preserving the filtration. These form a category.
\end{definition}
...
\begin{definition}\label{prereq:def:exhaustive-hausdorff}
    \uses{prereq:def:filtered-complex}
    A filtered chain complex $(C_*, d, F^s)$ is
    \begin{itemize}
        \item \emph{exhaustive} if $\bigcup_s F^s C_n = C_n$ for all $n$,
        \item \emph{Hausdorff} (or \emph{separated}) if $\bigcap_s F^s C_n = 0$ for all $n$,
        \item \emph{bounded} if for each $n$, there exist $a \le b$ with $F^a C_n = C_n$ and $F^{b+1} C_n = 0$.
    \end{itemize}
    A bounded filtration is both exhaustive and Hausdorff. We assume all filtered complexes in this project are exhaustive and Hausdorff.
\end{definition}
...
\begin{theorem}\label{prereq:thm:filtered-complex-convergence}
    \uses{prereq:thm:filtered-complex-ss,prereq:def:convergence,prereq:prop:homology-filtration,prereq:def:exhaustive-hausdorff}
...
\begin{proposition}\label{prereq:prop:ess-exactness}
    \uses{prereq:prop:f-ext-characterization,prereq:cor:composition-zero}
...
\subsubsection{Extension Differentials and Crossing}\label{sec:prereq-ess-crossing}
...
\begin{definition}\label{prereq:def:stable-homotopy-category}
    \uses{prereq:def:triangulated-category}
    The \emph{stable homotopy category} $\mathcal{S}$ is a symmetric monoidal triangulated category. Its objects are called \emph{spectra}. Homotopy pullback squares and homotopy pushout squares in $\mathcal{S}$ coincide.
\end{definition}

\begin{remark}
    In this project, $\mathcal{S}$ refers to the homotopy category of spectra (in the sense of stable homotopy theory). It can be constructed as the homotopy category of a stable model category, but we take it as given and work axiomatically with its triangulated structure.
\end{remark}
...
\begin{definition}\label{prereq:def:smash-product}
    \uses{prereq:def:stable-homotopy-category,prereq:def:sphere-Sn}
    The \emph{smash product} $\wedge: \mathcal{S} \times \mathcal{S} \to \mathcal{S}$ is a symmetric monoidal product with unit $\mathbb{S}$. It satisfies $S^m \wedge S^n \simeq S^{m+n}$ and induces a pairing on homotopy groups:
    $$\pi_m X \otimes \pi_n Y \to \pi_{m+n}(X \wedge Y).$$
\end{definition}

\begin{definition}\label{prereq:def:mapping-spectra}
    \uses{prereq:def:stable-homotopy-category,prereq:def:smash-product}
    For spectra $X, Y \in \mathcal{S}$, the \emph{mapping spectrum} $F(X, Y)$ is the internal hom adjoint to the smash product:
...
\begin{axiom}\label{prereq:ax:adams-e2-sphere}
    \uses{prereq:ax:adams-ss,prereq:def:sphere-spectrum}
    The $E_2$-page of the Adams spectral sequence for the sphere spectrum is a bigraded $\bF_2$-vector space $E_2^{s,t}(S^0)$, whose structure is computable and taken as given data.
\end{axiom}
...
\begin{definition}\label{prereq:def:synthetic-category}
    \uses{prereq:def:stable-homotopy-category}
    The category $h\mathrm{Syn}$ of \emph{$\HF$-synthetic spectra} is a triangulated category (it arises as the homotopy category of a stable model category $\mathrm{Syn}_{\HF}$).
\end{definition}
