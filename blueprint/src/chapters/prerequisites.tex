...
\begin{definition}\label{prereq:def:connective}
    \uses{prereq:def:connected}
    A spectrum $X$ is \emph{connective} if it is $n$-connected for some $n \in \bZ$.
\end{definition}
...
\begin{axiom}\label{prereq:ax:finite-type-char}
    \uses{prereq:def:finite-type,prereq:def:connective,prereq:def:homotopy-groups}
    A spectrum $X$ is of finite type if and only if:
    \begin{enumerate}
        \item $X$ is connective, and
