...
\begin{definition}\label{prereq:def:stable-homotopy-category}
    \uses{prereq:def:closed-symmetric-tensor-triangulated}
    The \emph{stable homotopy category} $\mathcal{S}$ is a closed symmetric tensor triangulated category (Definition~\ref{prereq:def:closed-symmetric-tensor-triangulated}). Its objects are called \emph{spectra}. Homotopy pullback squares and homotopy pushout squares in $\mathcal{S}$ coincide.
\end{definition}
