...
\subsubsection{Finite Spectra}\label{sec:prereq-finite-spectra}
...
\subsubsection{Connectivity}\label{sec:prereq-connectivity}
...
\begin{theorem}\label{prereq:thm:sphere-connected}
...
\begin{theorem}\label{prereq:thm:connected-homology-vanishing}
    \uses{prereq:def:connected,prereq:def:mod2-homology}
    If $X$ is $n$-connected, then $H_i(X; \bF_2) = 0$ for all $i \le n$.
\end{theorem}

\subsubsection{Finite Type}\label{sec:prereq-finite-type}
...
\begin{theorem}\label{prereq:thm:finite-type-char}
...
\begin{theorem}\label{prereq:thm:finite-type-homology}
...
\subsubsection{Adams Convergence and Boundedness}\label{sec:prereq-adams-convergence}

In this subsection, assume $X \in \calS^{fin}$ and $Y$ is of finite type.
...
\begin{theorem}\label{prereq:thm:not-odd-order-af-finite}
    \uses{prereq:def:adams-filtration-infinity}
    If $f$ does not have odd order, then $f$ does not have Adams filtration $\infty$.
\end{theorem}

\begin{theorem}\label{prereq:thm:adams-weak-convergence}
...
\begin{theorem}\label{prereq:thm:adams-separated}
    \uses{prereq:thm:adams-weak-convergence,prereq:thm:odd-order-af-infinity}
    The quotient $[X, Y] / \{\text{odd-order elements}\}$ carries an induced Adams filtration that is separated (Hausdorff).
\end{theorem}

\begin{theorem}\label{prereq:thm:adams-boundedness}
